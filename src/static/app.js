                ...
                ${details.participants.map(p => `
                  <li class="participant-item">
                    ...
                    <button class="delete-participant" title="Remove participant" data-activity="${name}" data-email="${p}">
                      <svg class="delete-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="10" fill="#ffebee"/><path d="M7 7l6 6M13 7l-6 6" stroke="#c62828" stroke-width="2" stroke-linecap="round"/></svg>
                    </button>
                  </li>
                `).join("")}
              </ul>
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        signupForm.reset();
        fetchActivities(); // Rafraîchir la liste des activités/participants
      } else {
